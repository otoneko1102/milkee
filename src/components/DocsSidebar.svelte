<script lang="ts">
  ...
  // Safe wrapper to compute option labels. Parent may not be able to pass a function
  // across SSR boundaries, so only call the prop if it's callable and otherwise
  ...
  const createId = (text: string) => {
    ...
    if (/[^\x00-\x7f]/.test(base)) return `h-${hashString(s)}`;
    return base;
  };
